...
public class CuentaServiceImpl implements ICuentaService {
  ...
  static final Logger logger = LoggerFactory.getLogger(CuentaServiceImpl.class);
  ...
  public CuentasResponse insertCuenta(CuentasRequest cuentasRequest) {
    ...
    try {
      if (!cuentaRepository.findByCuenta(cuentasRequest.getCuenta()).isEmpty()) {
        ...
        String description =
            (cuentasRequest.getDescription() == null || cuentasRequest.getDescription().isEmpty())
                ? "Descripción por defecto"
                : cuentasRequest.getDescription();
        CuentaTB nuevaCuenta = new CuentaTB(cuentasRequest.getCuenta(), description);
        cuentaRepository.save(nuevaCuenta);
        logger.info("cuenta guardada");
        cuentasResponse.setMensaje(
            "La cuenta " + cuentasRequest.getCuenta() + " se inserto correcto");
        return cuentasResponse;
      }
