...
public class CuentasController {
    ...
    @Autowired
    private ExternalMicroserviceService externalMicroserviceService;
    
    @PostMapping("/busquedas")
    public ResponseEntity<BusquedaCuentaResponse> buscarCuentas(
            @RequestHeader("x-id-acceso") @NotBlank(message = "x-id-acceso es requerido") String xIdAcceso,
            @RequestHeader("x-aplicacion-origen") @NotBlank(message = "x-aplicacion-origen es requerido") String xAplicacionOrigen,
            @Valid @RequestBody BusquedaCuentaRequest request) {
        
        BusquedaCuentaResponse response = externalMicroserviceService.procesarBusquedaCuenta(request, xIdAcceso);
